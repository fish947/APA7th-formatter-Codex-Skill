...
Use the bundled Python engine to modify an existing manuscript copy. AI supplies semantic judgments and examines rendered pages; the scripts enforce input binding, record completeness, supported preservation checks, and a conservative author-year correspondence check. Reusable APA7 Word styles are optional. Neither layer guarantees that every APA rule is satisfied.
...
2. **Classify and review.** Fill a role and a brief concrete reason for every paragraph and top-level table. Then complete every item in `compliance_review`: target requirements, title page, abstract/keywords, headings, citations/references, tables, figures and appendices. Use `pass` only when the manuscript provides enough evidence, `needs_review` when author or visual confirmation remains, and `not_applicable` only when the component is genuinely absent. Use content, neighboring elements, document hierarchy and actual visual evidence together. Do not equate bold text with a heading, a Word table with a data table, or a screenshot with an editable chart. Mark ambiguous elements `uncertain` and `preserve`, record the unresolved question, and proceed only with unambiguous work. Ask the user when the unresolved choice materially affects the result.
3. **Apply through the reviewed entrypoint.** Set `review_status` to `reviewed` only after doing the inspection, classification and compliance review. Run `apa7_workflow.py apply`, not the bare engine as a way to bypass a failed review check. It requires matching source hash/profile, complete classifications and reasons, complete compliance-review coverage, explicit data-table header counts, and preservation of uncertain or unsupported complex tables. After save, the engine reopens the DOCX and verifies supported margins, paragraph formatting, typography, table rules, header fields, and reusable styles when requested; a core mismatch stops delivery. It also reports likely unmatched in-text citations and reference entries without changing bibliography text. Treat these author-year matches as review hints, not corrections. Read the concise feedback returned by the command and inspect unresolved warnings; successful execution is not a compliance certificate.
